import { z } from "zod";
import { KEYWORDS_TO_ITEM, MENU_LOOKUP, RESTAURANT_CONFIG } from "../data/menu.js";
...
function processMessage(message, sessionId) {
    ...
    return {
        reply: "I can help you build an order from our menu. Tap on an item or ask me to add a dish like \"Add the Hot Stuff burger\".",
        cart: formatCart(session.cart)
    };
}
...
function findItemId(lowerMessage) {
...
function detectQuantity(lowerMessage) {
...
function handleAddition(session, itemId, lowerMessage) {
...
function handleRemoval(session, itemId, lowerMessage) {
